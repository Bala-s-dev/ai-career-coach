    ...
    <div className="min-h-screen" style={{ background: 'var(--ink)' }}>
      ...
      <section className="hero-grid relative min-h-screen flex items-center justify-center px-6 pt-16">
        ...
        <div className="relative max-w-4xl mx-auto text-center">
          ...
          >
            <span className="w-1.5 h-1.5 rounded-full pulse-gold" style={{ background: 'var(--gold)' }} />
          ...
          <p className="text-lg md:text-xl max-w-xl mx-auto mb-10 leading-relaxed" style={{ color: 'var(--mist)' }}>
            AI-powered resume analysis, personalized job matching, and live interview coaching — all in one elegant platform.
          </p>
          ...
          <div className="mt-20 flex flex-wrap items-center justify-center gap-12">
            {stats.map(({ value, label }) => (
              <div key={label} className="text-center">
                <div className="font-display text-4xl font-semibold text-gold-gradient">{value}</div>
                <div className="text-sm mt-1" style={{ color: 'var(--mist)' }}>{label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="features" className="px-6 py-28 max-w-6xl mx-auto">
        <div className="text-center mb-16">
          <p className="text-sm font-medium tracking-widest uppercase mb-4" style={{ color: 'var(--gold-dim)' }}>
            What We Offer
          </p>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {features.map(({ icon, title, description }, i) => (
            ...
            >
              ...
              <div>
                <h3 className="font-semibold text-lg mb-2" style={{ color: 'var(--cream)' }}>{title}</h3>
                <p className="text-sm leading-relaxed" style={{ color: 'var(--mist)' }}>{description}</p>
              </div>
            </div>
          ))}
        </div>
          ...
          <div className="space-y-6">
            {[
              ...
              <div key={num} className="flex gap-6 items-start card p-6">
                ...
                <div className="divider-vertical w-px self-stretch" style={{ background: 'var(--ink-5)' }} />
                <div>
                  <h4 className="font-semibold mb-1" style={{ color: 'var(--cream)' }}>{title}</h4>
                  <p className="text-sm" style={{ color: 'var(--mist)' }}>{body}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        >
          ...
          <div className="relative">
            ...
            <p className="mb-8" style={{ color: 'var(--mist)' }}>
              Join thousands of professionals who've elevated their career with ApexAI.
            </p>
            <a
              href={`${apiUrl}/auth/google`}
      ...
      {/* Footer */}
      <footer className="px-6 py-8" style={{ borderTop: '1px solid var(--ink-4)' }}>
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <div className="w-5 h-5 rounded flex items-center justify-center" style={{ background: 'var(--gold)' }}>
          ...
          <p className="text-xs" style={{ color: 'var(--mist)' }}>
            © 2025 ApexAI. Helping careers reach their peak.
          </p>
        </div>
      </footer>
    </div>
